...
class PG(object):
  ...
  def add_placeholders_op(self):
    ...
    self.observation_placeholder = tf.placeholder(tf.float32,shape=(None,self.observation_dim))  # TODO
    if self.discrete:
      self.action_placeholder = tf.placeholder(tf.int32,shape=(None,))# TODO
    else:
      self.action_placeholder = tf.placeholder(tf.float32,shape=(None,self.action_dim)) #self.action_dim TODO
  ...
  def build_policy_network_op(self, scope = "policy_network"):
    ...
    if self.discrete:
      ...
      #empty instead of self.config.activation  # TODO
      self.sampled_action = tf.squeeze(tf.multinomial(action_logits,1),axis=1) #self.config.batch_size # TODO
      ...
      action_means = build_mlp(self.observation_placeholder,self.action_dim,scope,
                                self.config.n_layers,self.config.layer_size) #self.config.activation  # TODO
      log_std = tf.get_variable(name=scope + '_std', shape=(self.action_dim,), dtype=tf.float32) #initializer=tf.zeros_initializer(), trainable=True
      self.sampled_action =  tf.random_normal(shape = (self.action_dim,),mean=action_means,stddev=tf.exp(log_std)) # TODO
      MVND = tf.contrib.distributions.MultivariateNormalDiag(loc=action_means,scale_diag = tf.exp(log_std))        # TODO
      self.logprob = MVND.log_prob(self.action_placeholder)
  ...
  def add_optimizer_op(self):
    ...
    ######################################################
    #########   YOUR CODE HERE - 1-2 lines.   ############
    optimizer = tf.train.AdamOptimizer(learning_rate=self.lr)# TODO
  ...
  def add_baseline_op(self, scope = "baseline"):
    ...
    self.baseline = tf.squeeze(build_mlp(self.observation_placeholder,1,scope,self.config.n_layers,
                              self.config.layer_size),axis=1)# TODO
    ...
    self.update_baseline_op = tf.train.AdamOptimizer(self.lr).minimize(loss)# TODO
    #######################################################
    #########          END YOUR CODE.          ############
  ...
  def get_returns(self, paths):
    ...
    for path in paths:
      ...
      returns_paths = [np.sum(np.power(gamma, np.arange(rewards_len - t)) * rewards[t:]) for t in range(rewards_len)]

      #######################################################
      #########          END YOUR CODE.          ############
      all_returns.append(returns_paths)
  ...
  def train(self):
    ...
    for t in range(self.config.num_batches):
      ...
      if self.config.use_baseline:
        ...
        # print('advantage shape = {}\n action shape = {}\n obsevation shape = {}'.format(advantages.shape,actions.shape,observations.shape))
