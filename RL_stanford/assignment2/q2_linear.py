...
class Linear(DQN):
    ...
    def add_placeholders_op(self):
        ...
        # print('state shape = {}\nbatch size = {}'.format(state_shape,batch_size))
        img_height, img_width,nchannels = state_shape
        channel_dim = nchannels * self.config.state_history
        self.s = tf.placeholder(tf.uint8,shape = (None,img_height,img_width,channel_dim))
        self.a = tf.placeholder(tf.int32,shape = (None))
        self.r = tf.placeholder(tf.float32,shape = (None))
        self.sp = tf.placeholder(tf.uint8,shape = (None,img_height,img_width,channel_dim))
    ...
    def get_q_values_op(self, state, scope, reuse=False):
        ...
        out = layers.fully_connected(flatten_state, num_actions,reuse=reuse,scope=scope)
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        vars_q = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,scope = q_scope)
        vars_target_q = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,scope = target_q_scope)
        print('vars q length = {}'.format(len(vars_q)))
        for i in range(len(vars_q)):
            print('el {} size = {}'.format(i,vars_q[i].shape))
        to_group = [tf.assign(vars_target_q[i],vars_q[i]) for i in range(len(vars_q))]
        update_taget_op = tf.group(*to_group)
        self.update_target_op = update_taget_op
        ##############################################################
        ######################## END YOUR CODE #######################
    ...
    def add_loss_op(self, q, target_q):
        ...
        q_samp = r + gamma * tf.cast(tf.logical_not(self.done_mask), tf.float32) * tf.reduce_max(target_q,axis = 1)
        a_one_hot = tf.one_hot(self.a, depth=num_actions)
        q_sa = tf.reduce_sum(tf.multiply(a_one_hot,q),axis = 1)
        loss = tf.reduce_mean(tf.square(q_samp - q_sa))
    ...
    def add_optimizer_op(self, scope):
        ...
        clipped_grads_and_vars  = optimizer.compute_gradients(self.loss,var_list = vars)
        if self.config.grad_clip:
            clipped_grads_and_vars  =[(tf.clip_by_norm(grad,self.config.clip_val),var) for grad,var in clipped_grads_and_vars ]

        grads = [grad for grad,var in clipped_grads_and_vars if grad is not None]
